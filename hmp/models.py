...
class hmp:
    ...
    def __init__(self, data, epoch_data=None, sfreq=None, cpus=1, event_width=50, shape=2, template=None, min_shape=None, distribution='gamma', corr_threshold=None, estimate_shape=False):
        ...
        for trial in range(self.n_trials):
            ...
            self.data_matrix[:self.durations[trial],trial,:] = \
                data.data.T[self.starts[trial]:self.ends[trial]+1,:]
    ...
    def fit_single(self, n_events=None, magnitudes=None, parameters=None, parameters_to_fix=None, 
                   ...
                   starting_points=1, method='random', return_max=True, verbose=True, cpus=None, min_shapes=None):
        '''
        ...
        min_shapes : ndarray
            1D ndarray n_events + 1, minimum shape per stage. If None (default), default shape will be used in EM(), based 
            on self.shape and/or corr_threshold.
        '''
        ...
        if isinstance(min_shapes, np.ndarray):
            min_shapes = min_shapes.copy()   
        ...
        if starting_points > 1 or len(np.shape(magnitudes)) == 3 or len(np.shape(parameters)) == 3:
            ...
            if cpus>1: 
                inputs = zip(magnitudes, parameters, itertools.repeat(maximization),
                        itertools.repeat(magnitudes_to_fix),itertools.repeat(parameters_to_fix), itertools.repeat(max_iteration), itertools.repeat(tolerance), itertools.repeat(min_iteration), itertools.repeat(None), itertools.repeat(None),itertools.repeat(None), itertools.repeat(1), itertools.repeat(min_shapes))
                ...
                for pars, mags in zip(parameters, magnitudes):
                    ...
                    magnitudes_to_fix, parameters_to_fix, max_iteration, tolerance, min_iteration, min_shapes=min_shapes))
                resetwarnings()
            ...
            min_shapes_sp = [x[4] for x in estimates]
            traces_sp = [x[5] for x in estimates]
            min_shapes_dev_sp = [x[6] for x in estimates]
            ...
            if return_max:
                ...
                min_shapes = min_shapes_sp[max_lkhs]
                traces = traces_sp[max_lkhs]
                min_shapes_dev = min_shapes_dev_sp[max_lkhs]
                ...
                min_shapes = min_shapes_sp
                traces = np.zeros((len(lkh),  max([len(x) for x in traces_sp])))*np.nan
                for i, _i in enumerate(traces_sp):
                    traces[i, :len(_i)] = _i
                min_shapes_dev = np.zeros((len(lkh),  max([len(x) for x in min_shapes_dev_sp]),n_events + 1 ))*np.nan
                for i, _i in enumerate(min_shapes_dev_sp):
                    min_shapes_dev[i, :len(_i), :] = _i
            ...
            lkh, mags, pars, eventprobs, min_shapes, traces, min_shapes_dev, param_dev = self.EM(initial_m, initial_p,\
                                         maximization, magnitudes_to_fix, parameters_to_fix, \
                                         max_iteration, tolerance, min_iteration, min_shapes=min_shapes)
            ...
            if np.any(magnitudes)== None:
                magnitudes = np.zeros((n_events, self.n_dims), dtype=np.float64)
            lkh, mags, pars, eventprobs, min_shapes, traces, min_shapes_dev, param_dev = self.EM(magnitudes, parameters, maximization, magnitudes_to_fix, parameters_to_fix,\
                                        max_iteration, tolerance, min_iteration, min_shapes=min_shapes)
        ...
        if len(np.shape(eventprobs)) == 3:
            ...
            xrmin_shapes_dev = xr.DataArray(min_shapes_dev, dims=("em_iteration", "stage"), name="min_shapes_dev", coords={'em_iteration':range(len(min_shapes_dev)),'stage':range(n_event_xr+1)})
            ...
            xrmin_shapes = xr.DataArray(min_shapes, dims=("stage"), name="min_shapes", 
            ...
            xrmin_shapes = xr.DataArray(min_shapes, dims=("iteration","stage"), name="min_shapes", 
                            coords = {'iteration': range(len(lkh))})
        estimated = xr.merge((xrlikelihoods, xrparams, xrmags, xreventprobs, xrmin_shapes, xrtraces, xrmin_shapes_dev, xrparam_dev))
    ...
    def fit_single_conds(self, magnitudes=None, parameters=None, parameters_to_fix=None, 
        ...
        locations : ndarray
            ...
            default location will be used in EM(), based on self.location and/or corr_threshold.
        '''
        #Conditions
    ...
    def EM(self, magnitudes, parameters, maximization=True, magnitudes_to_fix=None, parameters_to_fix=None, max_iteration=1e3, tolerance=1e-4, min_iteration=1, mags_map=None, pars_map=None,conds=None, cpus=1, min_shapes=None):  
        ...
        min_iteration: int
            Minimum number of iteration for the expectation maximization in the EM() function
        min_shapes : ndarray
            Minimum shape parameter for stages (1D for normal model, 2D for condition based model (cond * n_events + 1)
        ...
        min_shapes : ndarray
            min_shapes for each event
        traces: ndarray
            Values of the log-likelihood for each EM iteration
        min_shapes_dev : ndarray
            min_shapes for each interation of EM
        param_dev : ndarray
        ...
        if min_shapes is None:
            min_shapes = np.zeros((n_events+1,)) #,dtype=int) #minimum shape per stage
            min_shapes[0:] = self.min_shape #default is min_shape
            if n_cond is not None:
                min_shapes = np.tile(min_shapes, (n_cond, 1))
        ...
        traces = [lkh]
        min_shapes_dev = [min_shapes.copy()] #store development of min_shapes
        ...
        if not maximization:
            ...
            while i < max_iteration :#Expectation-Maximization algorithm
                if self.corr_threshold is None: #standard threshold
                ...
                else: #threshold adapted for correlation threshold:
                      #EM only stops if min_shapes was not changed on last iteration
                      #and events moved less than .1 sample. This ensures EM continues
                      #when new min_shapes are set or correlation is still too high.
                      #(see also get_magnitudes_parameters_expectation)
                    ...
                    if i >= min_iteration and (np.isneginf(lkh) or (min_shapes == min_shapes_prev).all() and tolerance > (lkh-lkh_prev)/np.abs(lkh_prev)):
    ...
    def get_magnitudes_parameters_expectation(self, eventprobs, parameters, parameters_prev, min_shapes, subset_epochs=None):
        ...
        for event in range(n_events):
            for comp in range(self.n_dims):
                magnitudes[event,comp] = np.median(np.sum(eventprobs[:,:,event]*self.data_matrix[:,subset_epochs,comp], axis=0))
    ...
    def update_parameters_correlation(self, min_shapes, parameters, parameters_prev, eventprobs):
        ...
        if n_events > 1 and not np.all(np.isclose(parameters[:,1], parameters[0,1])): #if not on first iteration
            ...
            stage_durations_prev = np.array([self.scale_to_mean(x[0],x[1]) for x in parameters_prev[1:,:]])
    ...
    def estim_probs(self, magnitudes, parameters, n_events=None, subset_epochs=None, lkh_only=False, by_trial_lkh=False):
        '''
        parameters
        ----------
        magnitudes : ndarray
        ...
        likelihood = likelihood + np.mean(lkh_dist)
        ...
        if cpus > 1:
            with mp.Pool(processes=cpus) as pool:
                likes_events_cond = pool.starmap(self.estim_probs, 
                    zip([magnitudes[c, mags_map[c,:]>=0, :] for c in range(n_conds)], [parameters[c, pars_map[c,:]>=0, :] for c in range(n_conds)], itertools.repeat(None), [conds == c for c in range(n_conds)], itertools.repeat(False)))
        else:
            for c in range(n_conds):
    ...
    def get_parameters(self, parameters, eventprobs, subset_epochs=None, min_shapes=None):
        ...
        ----------
        eventprobs : ndarray
            [samples(max_d)*n_trials*n_events]
        parameters : ndarray
        ...
        event_times = np.concatenate((np.arange(self.max_d) @ np.swapaxes(eventprobs,0,1) / np.sum(eventprobs,axis=0), np.expand_dims(self.durations[subset_epochs]-1,axis=1)),axis=1)
        durations = np.diff(event_times, prepend=0)
        ...
        for ev in range(durations.shape[1]):
            ...
            if self.estimate_shape == True:
                
                shape_step_size = self.mean_to_shape(self.event_width_samples, parameters[ev,1])/2
                default_lkh = self.sp_dist.logpdf(durations[:,ev], parameters[ev,0], loc=0, scale=parameters[ev,1]).sum()

                #current shape = min_shape => can only try up
                shape_up = parameters[ev,0] + shape_step_size
                scale_up = np.max([self.mean_to_scale(durations_mean[ev], shape_up), scale_lim])
                up_lkh = self.sp_dist.logpdf(durations[:,ev], shape_up, loc=0, scale=scale_up).sum()

                #if current shape > min_shape, also down
                down_lkh = -np.inf
                if parameters[ev,0] > min_shapes[ev]:
                    shape_down = np.max([parameters[ev,0] - shape_step_size, min_shapes[ev]])
                    scale_down = np.max([self.mean_to_scale(durations_mean[ev], shape_down),scale_lim])
    ...
    def compute_times(init, estimates, duration=False, fill_value=None, mean=False, mean_in_participant=True, cumulative=False, add_rt=False, extra_dim=None, as_time=False, errorbars=None, center_measure='mean',estimate_method='max', onset=False):
        ...
        if estimate_method == "max":
            ...
            times = xr.dot(eventprobs, eventprobs.samples, dims='samples') / np.sum(eventprobs,axis=1) - event_shift
    ...
    def compute_topologies(epoch_data, estimated, init, extra_dim=None, mean=True, mean_in_participant=True, peak=True, estimate_method='max'):
        ...
        if not extra_dim or extra_dim == 'condition': #also in the condition case, only one fit per trial
            if estimate_method == "max":
                ...
                times = np.round(xr.dot(estimated, estimated.samples, dims='samples') / np.sum(estimated,axis=1)) - event_shift
        ...
        while time == 0 or (self.scale_to_mean(last_stage, pars_prop[-1,0]) >= self.event_width_samples and n_events <= max_event_n):
            ...
            if sol_lkh - lkh_prev > 0:#accept solution if likelihood improved
                ...
                if not by_sample: #find furthest explored param. Note: this also work by_sample, just a tiny bit faster this way
                    prev_sample = int(np.round(np.sum(self.scale_to_mean(pars[:n_events-1,1],pars[:n_events-1,0]))))
                    max_sample = np.max([int(np.round(np.sum(self.scale_to_mean(x[:n_events,1],x[:n_events,0])))) for x in solutions.param_dev.values])
