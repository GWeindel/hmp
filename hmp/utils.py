...
def event_times(estimates, init=None, duration=False, fill_value=None, mean=False, mean_in_participant=True, add_rt=False, extra_dim=None, as_time=False, errorbars=None, center_measure='mean',estimate_method='max'):
    '''
    ...
    if extra_dim == 'levels':
        times_level = times.groupby('levels').mean('trial_x_participant').values #take average to make sure it's not just 0 on the trial-level
    ...
    if mean:
        ...
        if extra_dim == 'levels':
            ...
            if errorbars == 'std':
                ...
                raise ValueError("Unknown error bars, 'std' is for now the only accepted argument in the multilevel models")
            # elif errorbars == 'se':
...
def event_topo(epoch_data, estimated, extra_dim=None, mean=True, mean_in_participant=True, peak=True, estimate_method='max', template=None):
    """
    ...
    common_trials = np.intersect1d(estimated['trial_x_participant'].values, epoch_data['trial_x_participant'].values)
    ...
    if not extra_dim or extra_dim == 'levels': #also in the level case, only one fit per trial
        ...
        if extra_dim == 'levels':
            event_values = event_values.assign_coords(levels=("trial_x_participant", times.levels.data))
